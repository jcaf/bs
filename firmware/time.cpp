...
void mSetTime_do(void)
{
    ...
    if (thismenu_sm0 == 0)
    {
        ...
        lcd.clear();
        sysPrint_lcd_serialPrintln_P(0,0,PSTR("[SET TIME]"));
        sysPrint_lcd_serialPrintln_P(1,6,PSTR("(hh:mm)"));
...
static void mSetTime_print_hh(uint8_t hh)
{
    ...
    itoa(hh, str, 10);
    lcd.printAtPosc(2,col,str);//String(hh)
}
...
static void mSetTime_print_mm(uint8_t mm)
{
    ...
    lcd.printAtPosc(2,col,str);//String(mm)
}
...
void mSetTime_save_job(void)
{
    ...
    char db_mSetTime_save[MENU_SETTIME_SAVE][LCDAN_STR_MAX_SIZE];

    lcdan_str_lineformat_align_P(&db_mSetTime_save[0][0],  PSTR("1] Yes"), 7);
    lcdan_str_lineformat_align_P(&db_mSetTime_save[1][0],  PSTR("1] No"), 7);

    if (thismenu_sm0 == 0)
    {
        kb_change_keyDo_pgm(mSetTime_save_fxKey);
        vscroll.init(db_mSetTime_save, MENU_SETTIME_SAVE, vscroll_dispShowBuff, 2, FEAT_MARK_ON, vscroll.base, vscroll.offset);
        sysPrintLCDan_XY_wlineClr_wEOL3dots_P(0,0, PSTR("[SET TIME]"));
        sysPrintLCDan_XY_wlineClr_wEOL3dots_P(1,LCDAN_STR_FORMAT_ALIGN_CENTER, PSTR("Exit saving Time?"));
        thismenu_sm0 = -1;
    }
