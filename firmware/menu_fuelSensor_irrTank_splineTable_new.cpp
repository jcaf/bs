...
void mFuelSensor_irrTank_spTable_new_job(void)//New Table
{
    ...
    {
        int8_t opt_dest;
        cod_ret = menu_generic_yesno(PSTR("[MESSAGE]"),PSTR("Save new node?"), &opt_dest );
    ...
    {
        int8_t opt_dest;
        cod_ret = menu_generic_yesno(PSTR("[MESSAGE]"),PSTR("Enter new node?"), &opt_dest );
        if (cod_ret != MENU_GUI_EVENT_WAITING_ACTION)
        {
            if (cod_ret == MENU_GUI_EVENT_OPTION_SELECTED)
            {
    ...
    {
        int8_t opt_dest;
        cod_ret = menu_generic_yesno(PSTR("[MESSAGE]"),PSTR("Exit?"), &opt_dest);
        if (cod_ret != MENU_GUI_EVENT_WAITING_ACTION)
        {
            if (cod_ret == MENU_GUI_EVENT_OPTION_SELECTED)
            {
