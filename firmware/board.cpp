...
void BOARDd(void)
{
    ...
    if ( sm0 == 0 )
    {
        for (uint8_t num_board = 0; num_board < NUM_BOARD; num_board++)
        {
            ...
            board[num_board].stacks.num_max_stacks = BOARDd_SetUnionMode(num_board);//cada tarjeta puede tener 2 stacks separados o 1 stack total al unirse
    ...
    if (board[num_board].stack[board[num_board].stacks.num_stack].ltc_connected_last !=  ltc_connected[BOARD_get_num_ic_currentboard()])
    {
        ...
        boardD.AchangeInBoard = 1;//Provoca un cambio
        //Parece que como num_board es cambiado manualmente por el usuario al presionar up/down,  solo le compete al actual stack que se muestra en el LCD
        //por ahora lo cambio al ltc6804
        //ADDED 2020
//        if (ltc_connected[BOARD_get_num_ic_currentboard()] == LTC_DISCONNECTED )
//        {
//            //Clear all register LTC6804
//            ltc_stackFlush(BOARD_get_num_ic_currentboard());
//        }
    }
}
...
static uint8_t BOARDd_SetUnionMode(uint8_t num_board)
{
    uint8_t StackFormados_num_max= 2;//fallback ->por defecto tal vez se añada otra opciones... numero de opciones por cada board: stack lower y stack upper

    if (stack_union[ board[num_board].stack[StackUpper].address ] == 1)
    {
        if ( (ltc_connected[ board[num_board].stack[StackUpper].address ] == LTC_CONNECTED) && (ltc_connected[ board[num_board].stack[StackLower].address ] == LTC_CONNECTED))
        {
            StackFormados_num_max = 1;
            board[num_board].unionMode =  StackLowerUpper_UnitedByWire;
        }
