...
void mFuelSensor_rectangularVolume_job(void)
{
    if (thismenu_sm0 == 0)
    {
        ...
        lcd.clear();
        sysPrintLCDan_XY_wlineClr_wEOL3dots_P(0, 0, PSTR("[Dimensions in cm]"));
        sysPrintLCDan_XY_wlineClr_wEOL3dots_P(1, 3, PSTR("Length ="));
        sysPrintLCDan_XY_wlineClr_wEOL3dots_P(2, 3, PSTR("Width  ="));
...
void mFuelSensor_rectangularVolumen_save_and_exit_job(void)
{
    ...
    if (thismenu_sm0 == 1)
    {
        if (thismenu_sm1 == 0)
        {
            ...
            if ( width == 0.0f)
                strcat_P(msg, PSTR("[Width]"));

            sysPrintLCDan_XY_wlineClr_wEOL3dots(LCD_ROW_1, LCDAN_STR_FORMAT_ALIGN_CENTER, msg);
            sysPrintLCDan_XY_wlineClr_wEOL3dots_P(LCD_ROW_2, LCDAN_STR_FORMAT_ALIGN_CENTER, PSTR("must be > 0"));
            sysPrintLCDan_XY_wlineClr_wEOL3dots_P(LCD_ROW_3, LCDAN_STR_FORMAT_ALIGN_CENTER, PSTR("to get volume"));
    ...
    if (thismenu_sm0 >= 2)
    {
        ...
        if (thismenu_sm0 == 2)
        {
            if (thismenu_sm1 == 0)
            {
                //vscroll.init(db_vscroll, 2, vscroll_dispShowBuff, 2, FEAT_MARK_ON, vscroll.base, vscroll.offset);//base offset from Tank job
                vscroll.init(db_vscroll, 2, vscroll_dispShowBuff, 2, FEAT_MARK_ON, 0x00, 0x00);//without context push/pop
                ...
                if ( ( length == 0.0f) || ( width == 0.0f) )
                {
                    ...
                    sysPrintLCDan_XY_wlineClr_wEOL3dots_P(LCD_ROW_1, LCDAN_STR_FORMAT_ALIGN_CENTER, PSTR("Save data anaway?"));
                }
                else
                {
