...
void setnum_digit_blink_print_digit(char c)
{
    ...
    lcd.printAtPosc_char(fil, col, c);//String(c)
}
//
void setnum_charnumber_print_charvalue(uint8_t selected)
{
    ...
    lcd.printAtPosc(fil,col, setnum.charnumber.thenumber[selected].charvalue);
}
...
/*Example to use:
static uint8_t mAnInput_setFactors_setnum_number_printxy_getFil(uint8_t selected)
{
    uint8_t vfil[2] = {LCD_ROW_2,LCD_ROW_3};
    return vfil[selected];
}
static uint8_t mAnInput_setFactors_setnum_number_printxy_getCol(uint8_t selected)
{
    uint8_t vcol[2] = {6,7};
...
void f()
{
    if (thismenu_sm0 == 0)
    {
        ...
        //
        sysPrintLCDan_XY_wlineClr_wEOL3dots(LCD_ROW_1, 0, str);
        ...
        //step 2 :init
        setnum_charnumber_thenumber_digitposition_init(SCALE_SETNUM_CHARNUMBER_SELECTED, SCALE_ASCHAR_DIGPOS_DP, SCALE_ASCHAR_DIGPOS_ENDOFDIGIT);
        ...
        //step 3 :register xy
        setnum_printxy_register_pFil(mAnInput_setFactors_setnum_number_printxy_getFil);
        setnum_printxy_register_pCol(mAnInput_setFactors_setnum_number_printxy_getCol);
        ...
        setnum_charnumber_set_charvalue_asDou_aftFillZeros(SCALE_SETNUM_CHARNUMBER_SELECTED, , SCALE_ASCHAR_BUFFWITDH, SCALE_ASCHAR_NUMDECIMALS);
        setnum_charnumber_print_charvalue(SCALE_SETNUM_CHARNUMBER_SELECTED);

        setnum_charnumber_set_charvalue_asDou_aftFillZeros(OFFSET_SETNUM_CHARNUMBER_SELECTED, , OFFSET_ASCHAR_BUFFWITDH, OFFSET_ASCHAR_NUMDECIMALS);
