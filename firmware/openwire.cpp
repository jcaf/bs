...
uint8_t openwire_2v(uint8_t num_ic)
{
    ...
    if (sm0 == 1)
    {
        //ltc_write_cfg_address(num_ic, uv_2v+corfact_vars[FCOR_2V].value, ov_2v+corfact_vars[FCOR_2V].value, 0x0000 );//uv ov irrelevant
        ltc_write_cfg_address(num_ic, setacq_vars[UV_2V].value + corfact_vars[FCOR_2V].value, setacq_vars[OV_2V].value + corfact_vars[FCOR_2V].value, 0x0000 );//uv ov irrelevant
