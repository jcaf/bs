...
//static
void ltc_ifRemove_dataFlush(void);
...
{
    static int8_t ltc_connected_lock[NUM_IC];
    ...
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        ...
        if ( (count_presence == NUM_TOTAL_REGAUX )/*|| (vcellF[num_ic][0] <0.1)// || (vcellF[num_ic][11] < 0.1)*/)
        {
        ...
        else
        {
            ...
            ltc_connected_lock[num_ic] = 0;//clear lock
        }

        //--
        if (ltc_connected_lock[num_ic] == 0)
        {
            if ( ltc_connected[num_ic] == LTC_DISCONNECTED)
            {
                ltc_stackFlush(num_ic);    // 1 vez nada mas
                ltc_connected_lock[num_ic] = 1;
            }
        }
        //++
...
static void parse_gpio(uint16_t (*gpio_matrix)[NUM_TOTAL_REGAUX])//
{
    ...
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic] == LTC_CONNECTED)
        {
            ...
            temperatura = NTC10K_OUT_RANGE;//inicializo a "NTC10K_OUT_RANGE" por si cualquiera de los operandos es 0
            if (gpio_matrix[num_ic][AUX_GPIO_NTC10K] > 29700)//29890 //casi igual al valor ADC_TOP_VAL
            {
                temperatura = NTC10K_NO_CONNECTED;
            }
            else
            {
                if (gpio_matrix[num_ic][AUX_LTC_VREF2] > 0) //evitar div. por 0
                {
                    if (gpio_matrix[num_ic][AUX_GPIO_NTC10K] > 0) //evitar div. por 0
                    {
                        temperatura = temperature_read( gpio_matrix[num_ic][AUX_GPIO_NTC10K], gpio_matrix[num_ic][AUX_LTC_VREF2] ) + corfact_temperature[num_ic].adder;//add corfact each IC

                        if ( (temperatura < -40.0f) || (temperatura > 105.0f) )
                        {
                            temperatura = NTC10K_OUT_RANGE;
                        }
                    }
                }
            }
            stack_temperatureF[num_ic] = temperatura;
            //

            //4.- get stack current
            //stack_currentF[num_ic] = get_stack_current(gpio_matrix[num_ic][AUX_GPIO_CURRENTSENSE]);
            current = ( get_stack_current(gpio_matrix[num_ic][AUX_GPIO_CURRENTSENSE]) * corfact_shunt.multiplier ) + corfact_shunt.adder;//for all
            if (current <= 0.09f)
            {
                current = 0.0f;
            }
            stack_currentF[num_ic] = current;
            //
        }
...
/////////////////////////////////////////////////////////////////////////////////////
void ltc_stackFlush(uint8_t num_ic)
{
...
static uint8_t read_cellvoltage(void)
{
    ...
    if (ltc_connected[num_ic] == LTC_CONNECTED)
    {
        if (sm0 == 0)
        {
            if (select_2_12V[num_ic] == 2)
            {
                //ltc_write_cfg_address(num_ic, uv_2v+corfact_vars[FCOR_2V].value, ov_2v+corfact_vars[FCOR_2V].value, 0x0000);
                ltc_write_cfg_address(num_ic, setacq_vars[UV_2V].value+corfact_vars[FCOR_2V].value, setacq_vars[OV_2V].value+corfact_vars[FCOR_2V].value, 0x0000);
...
static void parse_cellvoltage(uint8_t num_ic, uint16_t data_read[LTC_NUM_CELLS])
{
    ...
    if (select_2_12V[num_ic] == 2)
    {
        ...
        for (int num_cell = 0; num_cell < LTC_NUM_CELLS; num_cell++)
        {
            vcellF[num_ic][num_cell] += corfact_vars[FCOR_2V].value;
    ...
    {
        float C1_12V = ((vcellF[num_ic][0]) ) + ((vcellF[num_ic][1]) ) + ((vcellF[num_ic][2]) ) + corfact_vars[FCOR_12V_C1].value;
        float C2_12V = ((vcellF[num_ic][3]) ) + ((vcellF[num_ic][4]) ) + ((vcellF[num_ic][5]) ) + corfact_vars[FCOR_12V_C2].value;
        float C3_12V = ((vcellF[num_ic][6]) ) + ((vcellF[num_ic][7]) ) + ((vcellF[num_ic][8]) ) + corfact_vars[FCOR_12V_C3].value;
        float C4_12V = ((vcellF[num_ic][9]) ) + ((vcellF[num_ic][10]) ) + ((vcellF[num_ic][11]) ) + corfact_vars[FCOR_12V_C4].value;
    ...
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic] == LTC_CONNECTED)
        {
            ...
            else
            {
                ...
                {
                    if (vcellF[num_ic][num_cell] < setacq_vars[UV_12V_S3C].value)
                        uv_flag[num_ic][num_cell] = 1;
                    else
                        uv_flag[num_ic][num_cell] = 0;

                    if (vcellF[num_ic][num_cell] > setacq_vars[OV_12V_S3C].value)
