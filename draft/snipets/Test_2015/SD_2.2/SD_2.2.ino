...
#include <SPI.h>
  ...
  #define NUM_IC 8//8
  #define NUM_CELL_MAX 12

  float FACT_CORR = +0.004; // for FLUKE 175

  uint16_t cell_voltage_matrix[NUM_IC][NUM_CELL_MAX]; //12 cell + SUMA TOTAL
  
  float cell_voltage_f_matrix[NUM_IC][NUM_CELL_MAX]; //12 cell + SUMA TOTAL
  float gpio_f_matrix[NUM_IC][6];
  uint8_t uv_flag_matrix[NUM_IC][NUM_CELL_MAX];
  uint8_t ov_flag_matrix[NUM_IC][NUM_CELL_MAX];
  uint8_t openwire_flag_matrix[NUM_IC][NUM_CELL_MAX];
...
//
/////////////////////////////////////////////////////////////////////////////////////
//void cell2float(uint16_t cell_voltage_matrix[][NUM_CELL_MAX])
void cell2float()
{
  for (int num_ic=0; num_ic<NUM_IC; num_ic++)
  {
      for (int num_cell=0; num_cell<NUM_CELL_MAX; num_cell++)
      {
    ...
    for (int num_ic=0; num_ic<NUM_IC; num_ic++)
    {
        ...
        if (ltc_connected_matrix[num_ic] == 1)
        {
          ...
          */
          ...
          for (int num_cell=0; num_cell<(NUM_CELL_MAX-1); num_cell++)//open wire for C1-C11
          {
