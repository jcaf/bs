...
uint8_t ADSTAT[2];//Set ADSAT
uint8_t ADOW[2];//Set ADOW
...
int8_t LTC6804_rdstat(char AB, uint8_t total_ic, uint8_t r_stat[][8])
{
  const uint8_t BYTES_IN_REG = 8;
  ...
  for (uint8_t current_ic = 0; current_ic < total_ic; current_ic++) //executes for each LTC6804 in the stack
  {
    ...
    for (uint8_t current_byte = 0; current_byte < BYTES_IN_REG; current_byte++)
    {
      r_stat[current_ic][current_byte] = rx_data[current_byte + (current_ic*BYTES_IN_REG)];
    }
    //4.b
    received_pec = (r_stat[current_ic][6]<<8) + r_stat[current_ic][7];
    data_pec = pec15_calc(6, &r_stat[current_ic][0]);
    if(received_pec != data_pec)
    {
      pec_error = -1;
    }
...
/*!******************************************************************************************************************
 \brief Maps control variables for the appropriate ADSTAT command
 ...
 ******************************************************************************************************************/
void set_stat(uint8_t MD, //ADC Mode
			 uint8_t CHST //Status Group Selection-> Ltc68042.h
			 )
{
...
/*!******************************************************************************************************************
 \brief Maps control variables for the appropriate ADOW command
 ...
 ******************************************************************************************************************/
void set_adow(uint8_t MD, //ADC Mode
                uint8_t PUP,
                uint8_t DCP,
			 uint8_t CH
			 )
