    ...
    else if (sm0 == 1)
    {
        if (millis() - millis_last >100)//20ms
        {
            if (link_status_changed == link_status)
            {
                ...
                {
                    sysPrint_serialPrintlnPSTR(PSTR("\t\t\t\t ETH_LINK_PIN_IS_UP") );
                ...
                {
                    sysPrint_serialPrintlnPSTR(PSTR("\t\t\t\t\ETH_LINK_PIN_IS_DOWN") );
...
#define ETH_INIT_DHCP_IP_WAIT_TIME_MAX 10000// 10s
        ...
        {
            sysPrint_serialPrintln("");
            sysPrint_lcd_serialPrintlnPSTR(2,STR_CENTER,PSTR("Fail IP config"));
            delay(SDCARD_BOOT_LCD_TIME2VIEW);
        }
    }
    else
    {
        sysPrint_serialPrintln("");
        sysPrint_lcd_serialPrintlnPSTR(2,STR_CENTER,PSTR("Error: Link"));
        delay(SDCARD_BOOT_LCD_TIME2VIEW);
    }
...
uint8_t eth_uploadData_job(void)
{
    ...
    if (eth_data2send_length > 0)
    {
        ...
        {
            sysPrint_serialPrintlnPSTR(PSTR("Conexion to server!"));
            ...
            client.println(F("User-Agent: BatterySystem/1.00"));
            //
...
uint8_t eth_uploadData_job1(void)
{
    ...
    if (sm0 == 0)
    {
        //eth_data2send_length =  eth_datasend_stackGetLength();
...
//            eth_server[3]= 38;
            ...
            if (client.connect(eth_server, 80))
            {
                ...
                client.print("/modules/arduino/listener/jsonListener");
                //client.print(mainvar_getvalue("Directory"));
//                client.print("/jsondecode.php");
                ...
                client.println("mvcnino.ddns.net");
//                client.println(mainvar_getvalue("Host"));//githubprototypes.battery.com"));//obligado for HTTP 1.1.
//                client.println("FIRWAR");//githubprototypes.battery.com"));//obligado for HTTP 1.1.
...
/*
uint8_t eth_uploadData_job11(void)
{
    uint8_t cod_ret = 0;
        ...
        if (sm0 == 1)
        {
            ...
            while ( (millis() - timecpu_last_millis) < ETH_RXDATA_FROM_SERVER_TIMECPU_READING)////read all data as posible
            {
                if (client.available())
                {
                    char rx= client.read();
                    //Serial.print(rx);
                    sysPrint_serialPrint(String(rx) );
                    if (rx == '?')
                    {
                        sm0=2;
                        break;
