...
void mFuelSensor_irrTank_menu_job(void)
{
    ...
    String db_mFuelSensor_irrTank_menu[MENU_FUELSENSOR_IRREGULAR_MAINMENU] =
    {
        str_clearPrint("1] UNITS OF TABLE", 0),
        str_clearPrint("2] NEW TABLE", 0),
        str_clearPrint("3] MODIFY NODE", 0),
        str_clearPrint("4] DELETE NODE", 0),
        str_clearPrint("5] INSERT NODE", 0),
    };
    if (thismenu_sm0 == 0)
    {
        kb_change_keyDo_pgm(mFuelSensor_irrTank_menu_fxKey);
        vscroll.init(db_mFuelSensor_irrTank_menu, MENU_FUELSENSOR_IRREGULAR_MAINMENU, dispShowBuff, 3, FEAT_MARK_ON, vscroll.base, vscroll.offset);
        lcd.clear();
        sysPrint_lcdPrintPSTR(LCD_ROW_0, 0, PSTR("[NODE TABLE]"));
